...
export default function InterviewPage() {
  ...
  const audioChunksRef = useRef<Blob[]>([])  const [uploadedFile, setUploadedFile] = useState<File | null>(null)
      ...
      <div className="max-w-2xl mx-auto px-6 py-12">
        ...
        {!isRecording ? (
          <button onClick={startFreeRecording} className="bg-purple-600 hover:bg-purple-700 text-white font-bold text-lg px-10 py-4 rounded-2xl transition w-full">
            🎙️ {t('Alusta salvestust', 'Start recording')}          </button>          <div className="relative flex items-center my-4"><div className="flex-grow border-t border-gray-700"></div><span className="mx-4 text-gray-500 text-sm">{t('või', 'or')}</span><div className="flex-grow border-t border-gray-700"></div></div>          <input ref={fileInputRef} type="file" accept="audio/*,.m4a,.mp3,.wav,.ogg,.webm" onChange={e => setUploadedFile(e.target.files?.[0] || null)} className="hidden" />          {!uploadedFile ? (            <button onClick={() => fileInputRef.current?.click()} className="bg-gray-800 hover:bg-gray-700 text-gray-300 font-bold text-lg px-10 py-4 rounded-2xl transition w-full">📁 {t('Lae üles helifail (.m4a, .mp3, .wav)', 'Upload audio file (.m4a, .mp3, .wav)')}</button>          ) : (            <div className="space-y-3"><p className="text-gray-400 text-sm text-center">✓ {uploadedFile.name}</p><button onClick={async () => { setStage('processing'); setProcessingMessage(t('Transkribeerin...', 'Transcribing...')); const fd = new FormData(); fd.append('audio', uploadedFile); fd.append('companyId', company!.id); fd.append('sessionNumber', String(sessionNumber)); fd.append('language', company!.language || 'et'); fd.append('freeRecording', 'true'); const res = await fetch('/api/transcribe', { method: 'POST', body: fd }); if (!res.ok) { setError(t('Viga üleslaadimise', 'Upload error')); setStage('error'); return; } setProcessingMessage(t('Genereerin raporti...', 'Generating report...')); const r2 = await fetch('/api/generate-report', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ companyId: company!.id, sessionNumber }) }); const d2 = await r2.json(); if (d2.summary) { setReport(d2.summary); setStage('report'); } else { setError('Viga raporti genereerimisel'); setStage('error'); } }} className="bg-purple-600 hover:bg-purple-700 text-white font-bold text-lg px-10 py-4 rounded-2xl transition w-full">🚀 {t('Saada transkriptsioonile →', 'Send for transcription →')}</button><button onClick={() => setUploadedFile(null)} className="text-gray-500 text-sm w-full text-center hover:text-gray-300">✕ {t('Vali teine fail', 'Choose different file')}</button></div>          </button>          <div className="relative flex items-center my-4"><div className="flex-grow border-t border-gray-700"></div><span className="mx-4 text-gray-500 text-sm">{t('või', 'or')}</span><div className="flex-grow border-t border-gray-700"></div></div>          <input ref={fileInputRef} type="file" accept="audio/*,.m4a,.mp3,.wav,.ogg,.webm" onChange={e => setUploadedFile(e.target.files?.[0] || null)} className="hidden" />          {!uploadedFile ? (            <button onClick={() => fileInputRef.current?.click()} className="bg-gray-800 hover:bg-gray-700 text-gray-300 font-bold text-lg px-10 py-4 rounded-2xl transition w-full">📁 {t('Lae üles helifail (.m4a, .mp3, .wav)', 'Upload audio file (.m4a, .mp3, .wav)')}</button>          </button>          <div className="relative flex items-center my-4"><div className="flex-grow border-t border-gray-700"></div><span className="mx-4 text-gray-500 text-sm">{t('või', 'or')}</span><div className="flex-grow border-t border-gray-700"></div></div>          <input ref={fileInputRef} type="file" accept="audio/*,.m4a,.mp3,.wav,.ogg,.webm" onChange={e => setUploadedFile(e.target.files?.[0] || null)} className="hidden" />          </button>
          </button>
        ) : (
