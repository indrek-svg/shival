...
export default function InterviewPage() {
        ...
        {!isRecording ? (<>
          ...
          {!uploadedFile ? (
            ...
            <div className="space-y-3">
              ...
              <button onClick={() => setUploadedFile(null)} className="text-gray-500 text-sm w-full text-center hover:text-gray-300">
                ✕ {t('Vali teine fail', 'Choose different file')}
              </button>
            </div>
          )}
        </>
  ...
  if (stage === 'transcript_ready') return (
    <div className="min-h-screen bg-gray-950 text-white">
      <div className="max-w-2xl mx-auto px-6 py-12">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-400 text-sm">{t('Lae alla enne raporti genereerimist', 'Download before generating report')}</p>
        </div>
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-4 mb-6 max-h-64 overflow-y-auto">
          <p className="text-gray-300 text-sm whitespace-pre-wrap leading-relaxed">{transcriptText || t('(Transkript on tühi)', '(Transcript is empty)')}</p>
        </div>
        <div className="space-y-3">
          <button onClick={() => { const blob = new Blob([transcriptText], {type:'text/plain'}); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href=url; a.download=(company?.company_name||'') + '_' + (company?.person_name||'') + '_transkript.txt'; a.click(); URL.revokeObjectURL(url); }} className="bg-gray-700 hover:bg-gray-600 text-white font-medium px-6 py-3 rounded-xl w-full flex items-center justify-center gap-2 transition">
            ⬇️ {t('Lae transkript alla (.txt)', 'Download transcript (.txt)')}
          </button>
          <button onClick={async () => { setStage('processing'); setProcessingMessage(t('Genereerin raporti...', 'Generating report...')); const r2 = await fetch('/api/generate-report', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ companyId: company!.id, sessionNumber }) }); const d2 = await r2.json(); if (d2.executiveSummary || d2.success) { if (d2.executiveSummary) setReport(d2.executiveSummary); const r3 = await fetch('/api/interview/' + slug); const d3 = await r3.json(); setNextSessionQuestions((d3.questions||[]).filter((q: Question) => q.session_number === 2)); setStage('report'); } else { setError('Viga raporti genereerimisel'); setStage('error'); } }} className="bg-blue-600 hover:bg-blue-700 text-white font-bold px-6 py-4 rounded-xl w-full transition">
