...
type Stage = 'loading' | 'preview' | 'welcome' | 'recording' | 'processing' | 'report' | 'error'
    ...
    <div className="min-h-screen bg-gray-950 text-white">
      <div className="max-w-2xl mx-auto px-6 py-12">
                ...
                <div className="flex items-start gap-3">
                  ...
                  {editingIndex === i ? (
                    <div className="flex-1">
                      <textarea
                        className="w-full bg-gray-700 text-white rounded-lg p-2 text-sm resize-none focus:outline-none focus:border-blue-500 border border-gray-600"
                        rows={3}
        ...
        <button onClick={() => setStage('welcome')} className="bg-blue-600 hover:bg-blue-700 text-white font-bold text-lg px-10 py-4 rounded-2xl transition w-full">
          {t('Alusta intervjuud →', 'Start interview →')}
        </button>
      </div>
    </div>
  )
        ...
        <div className="max-w-2xl w-full">
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-2xl p-8 mb-8">
            <p className="text-white text-xl leading-relaxed text-center">
              {currentQuestionIndex === editableQuestions.length
                ? t('Kas soovid ise midagi lisada mida me ei küsinud?', 'Is there anything you would like to add that we did not ask?')
                : q.question_text}
            </p>
          </div>
